...
public class ConnectDB {
	...
	public ConnectDB() {
		...
		try {
			...
			JOptionPane.showMessageDialog(null, "와이파이를 연결해주세요.", "네트워크 연결 실패",
	...
	public String checkInfo(String inputId, String inputPw, String inputName) {
		try {
			...
			String queryString = "SELECT * FROM userInfo where id = '"
					+ inputId + "'";
	...
	// 회원가입
	@SuppressWarnings("unused")
	public boolean checkJoinInfo(String inputId, String inputPw,
			String inputName) {
		try {
			...
			queryString = "INSERT INTO userInfo VALUES ('" + inputId + "', '"
					+ inputName + "', '" + inputPw + "')";
			...
			for (int i = 0; i < songTable.length; i++) {
				queryString = "INSERT INTO " + songTable[i] + " VALUES ('"
						+ inputId + "', 0)";
	...
	public void getSongRecord() {
		try {
			...
			String[] queryString = {
					"SELECT * FROM gangnamstyle ORDER BY score DESC",
					"SELECT * FROM D ORDER BY score DESC",
					"SELECT * FROM redflavor ORDER BY score DESC",
					"SELECT * FROM cracked ORDER BY score DESC" };
	...
	public boolean updateScore(String userScore) {
		try {
			...
			String queryString = "SELECT score FROM " + songName
					+ " where id = '" + RhythmTyping.playID + "'";
			...
			while (resultSet.next()) {
				...
				if (Integer.parseInt(score) < Integer.parseInt(userScore)) {
					queryString = "UPDATE " + songName + " SET score ='"
							+ userScore + "' WHERE id = '"
							+ RhythmTyping.playID + "'";

					int resultSet = statement.executeUpdate(queryString);

					RhythmTyping.playScore[SelectMusicPanel.songIndex] = Integer
							.parseInt(userScore);
				}
